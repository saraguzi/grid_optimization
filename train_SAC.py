...
if __name__ == "__main__":
    ...
    run_name: str = f"SAC_agent_{time_now}"
    ...
    trained_agents_dir = f"./rl_agents/trained_agents/sac_{int(time.time())}"
    ...
    # Save
    final_model_path = os.path.join(trained_agents_dir, "sac_final.zip")
    model.save(final_model_path)
    stats_path = os.path.join(trained_agents_dir, "vec_normalize_final.pkl")
    train_norm_vec_env.save(stats_path)

    # Evaluation
    eval_agent: Evaluation = BasicEvaluation(n_steps=n_eval_steps, n_evs=n_evs, n_episodes=n_eval_episodes, n_envs=1, algorithm="SAC")
