...
if __name__ == "__main__":
    ...
    run_name: str = f"DDPG_agent_{time_now}"
    ...
    eval_vec_env = make_vec_env(FleetEnv, n_envs=n_envs, vec_env_cls=SubprocVecEnv,
                                env_kwargs={"env_config": env_config}, seed=env_config["seed"] + 1)
    ...
    test_vec_env = make_vec_env(FleetEnv, n_envs=n_envs, vec_env_cls=SubprocVecEnv,
                                env_kwargs={"env_config": env_config}, seed=env_config["seed"] + 2)
    ...
    # Prepare action noise for DDPG
    action_dim = train_norm_vec_env.action_space.shape[0]
    ...
    trained_agents_dir = f"./rl_agents/trained_agents/ddpg_{int(time.time())}"
    ...
    # Save
    final_model_path = os.path.join(trained_agents_dir, "ddpg_final.zip")
    model.save(final_model_path)
    stats_path = os.path.join(trained_agents_dir, "vec_normalize_final.pkl")
    train_norm_vec_env.save(stats_path)

    # Evaluation
    eval_agent: Evaluation = BasicEvaluation(n_steps=n_eval_steps, n_evs=n_evs, n_episodes=n_eval_episodes, n_envs=1, algorithm="DDPG")
    rl_log = eval_agent.evaluate_agent(env_kwargs={"env_config": env_config}, norm_stats_path=stats_path,
                                       model_path=final_model_path, seed=env_config["seed"])

    uc: Benchmark = Uncontrolled(n_steps=n_eval_steps, n_evs=n_evs, n_episodes=n_eval_episodes, n_envs=1,
                                 time_steps_per_hour=time_steps_per_hour)
