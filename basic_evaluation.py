...
class BasicEvaluation(Evaluation):
    ...
    def __init__(self, n_steps: int, n_evs: int, n_envs: int = 1, n_episodes: int = 1, algorithm: str = "PPO"):
    ...
    def evaluate_agent(self,
        ...
        if self.algo.upper() == "SAC":
            model = SAC.load(
                model_path,
                env=eval_norm_vec_env,
                custom_objects={
                    "observation_space": eval_norm_vec_env.observation_space,
                    "action_space": eval_norm_vec_env.action_space
                }
            )

        elif self.algo.upper() == "DDPG":
            model = DDPG.load(
                model_path,
                env=eval_norm_vec_env,
                custom_objects={
    ...
    def compare(self, rl_log: pd.DataFrame, benchmark_log: pd.DataFrame, benchmark_name):
        ...
        print(f"{benchmark_name} reward: {benchmark_reward}")
        print(f"RL cashflow: {rl_cashflow}")
        print(f"{benchmark_name} cashflow: {benchmark_cashflow}")
        ...
        total_results[f"{benchmark_name}"] = [benchmark_reward,
                                               benchmark_cashflow,
        ...
        mean_both[f"{benchmark_name}"] = np.multiply(mean_all_benchmark, 4)
    ...
    def plot_soh(self, rl_log, benchmark_log, benchmark_name):
        ...
        # Plot the data
        ax.plot(rescaled_benchmark_log.index, rescaled_benchmark_log['SOH'].apply(lambda x: x[0]), label=benchmark_name, color='red')
    ...
    def plot_violations(self, rl_log):

        if len(rl_log[rl_log["SOC violation"] > 0]) == 0:
            print("No violations found.")
            return None

        pd.set_option("display.max_rows", None)  # zobrazí všetky riadky
        pd.set_option("display.max_columns", None)  # zobrazí všetky stĺpce
        pd.set_option("display.width", None)  # odstráni orezanie na šírku
        pd.set_option("display.max_colwidth", None)  # zobrazí celé bunky
        print(rl_log[rl_log["SOC violation"] > 0])
    ...
    def plot_action_dist(self, rl_log, benchmark_log, benchmark_name):

        if rl_log['Action'][0].__class__ == np.ndarray:
            rl_log['Action'] = rl_log['Action'].apply(lambda x: x[0])

        if benchmark_log['Action'][0].__class__ == np.ndarray:
        ...
        axs[0].hist(rl_log['Action'].values.flatten(), bins=50, color='blue', edgecolor='black')
        axs[0].set_title("RL-based Charging")
        axs[0].set_xlabel('Action')
        axs[0].set_ylabel('Frequency')

        # Plot the distribution of actions for the benchmark strategy on the second subplot
        axs[1].hist(benchmark_log['Action'].values.flatten(), bins=50, color='red', edgecolor='black')
        axs[1].set_title(benchmark_name)
    ...
    def plot_cost(self, rl_log, benchmark_log, benchmark_name):
        ...
        # Plot the data
        ax.plot(rescaled_benchmark_log.index, rescaled_benchmark_log['Cashflow'], label=benchmark_name, color='red')

        ax.plot(rescaled_rl_log.index, rescaled_rl_log['Cashflow'], label='RL', color='blue')

        # Set the title and labels
        ax.set_title('Cashflow Over Time')
        ax.set_xlabel('Time')
    ...
    def plot_cumulative_cost(self, rl_log, uc_log, dist_log, night_log):

        date_range = pd.date_range(
            start=rl_log["Time"].iloc[0],
            end=rl_log["Time"].iloc[-1],
            freq="15min"
        )

        def rescale_and_extract_cost(df):
            """Rescale index and extract Cashflow as float."""
            new_df = df.copy()
            new_df.index = date_range[:len(df)]

            cf = new_df["Cashflow"].apply(lambda x: x[0] if isinstance(x, (list, np.ndarray)) else x)

            # Return cumulative sum
            return cf.cumsum()

        rl_cost = rescale_and_extract_cost(rl_log)
        uc_cost = rescale_and_extract_cost(uc_log)
        dist_cost = rescale_and_extract_cost(dist_log)
        night_cost = rescale_and_extract_cost(night_log)

        # --- Plot ---
        fig, ax = plt.subplots(figsize=(12, 6))

        ax.plot(rl_cost.index, rl_cost, label="RL", linewidth=2.5)
        ax.plot(uc_cost.index, uc_cost, label="Uncontrolled charging", linestyle="--")
        ax.plot(dist_cost.index, dist_cost, label="Distributed charging", linestyle="--")
        ax.plot(night_cost.index, night_cost, label="Night charging", linestyle="--")
        ...
        print(f"RL:              {rl_cost.iloc[-1]:.2f} €")
        print(f"{"Uncontrolled charging"}:    {uc_cost.iloc[-1]:.2f} €")
        print(f"{"Distributed charging"}:    {dist_cost.iloc[-1]:.2f} €")
        print(f"{"Night charging"}:    {night_cost.iloc[-1]:.2f} €")

        print("\n===== SAVINGS vs HEURISTICS =====")
        print(f"RL vs {"Uncontrolled charging"}:  {uc_cost.iloc[-1] - rl_cost.iloc[-1]:.2f} €")
        print(f"RL vs {"Distributed charging"}:  {dist_cost.iloc[-1] - rl_cost.iloc[-1]:.2f} €")
        print(f"RL vs {"Night charging"}:  {night_cost.iloc[-1] - rl_cost.iloc[-1]:.2f} €")

    def plot_training_rewards(self, callback):
